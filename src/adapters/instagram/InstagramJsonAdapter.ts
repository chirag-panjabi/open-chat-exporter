...
function normalizeTimestampUtcFromMs(timestampMs: number): string {
  ...
  if (Number.isNaN(date.getTime())) throw new Error('Invalid timestamp_ms in Instagram export');
...
function mapInstagramMessageToUnified(message: Record<string, unknown>): IUnifiedMessage {
  const senderName = getString(message, ['sender_name']) ?? 'Unknown';
  const senderId = sha256Hex(`instagram|sender|${senderName}`);

  const timestampMs = getNumber(message, ['timestamp_ms']);
  if (typeof timestampMs !== 'number') {
    throw new Error('Missing timestamp_ms in Instagram message');
  }
  ...
  const messageId = sha256Hex(`instagram|${timestampUtc}|${senderName}|${content}`);
...
export class InstagramJsonAdapter extends BaseAdapter {
  ...
  async *parseMessages(input: AdapterInput): AsyncGenerator<IUnifiedMessage> {
    ...
    for await (const chunk of messagesArrayStream as AsyncIterable<StreamArrayChunk>) {
      ...
      yield mapInstagramMessageToUnified(chunk.value);
    }
  }
}
