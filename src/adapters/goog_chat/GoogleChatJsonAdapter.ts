...
function normalizeTimestampUtc(value: unknown): string {
  ...
  throw new Error('Invalid timestamp in Google Chat message');
}

function hasAnyAttachment(message: Record<string, unknown>): boolean {
  const arrays = ['attachments', 'attachment', 'attachedFiles', 'files', 'images', 'videos'];
  for (const key of arrays) {
    const value = message[key];
    if (Array.isArray(value) && value.length > 0) return true;
    if (!Array.isArray(value) && value != null && key !== 'attachment') return true;
  }
  return false;
}

function getCreatorName(message: Record<string, unknown>): string {
  const creator = getRecord(message, ['creator', 'sender', 'author']);
  if (creator) {
    const name = getString(creator, ['name', 'displayName', 'email']);
    if (name) return name;
  }

  const fallback = getString(message, ['creator_name', 'sender_name', 'from']);
  return fallback ?? 'Unknown';
}

function mapGoogleChatMessageToUnified(message: Record<string, unknown>): IUnifiedMessage {
  const senderName = getCreatorName(message);
  const senderId = sha256Hex(`goog_chat|sender|${senderName}`);

  const timestampRaw =
    message.createTime ??
    message.created_timestamp ??
    message.timestamp_ms ??
    getString(message, ['createTime', 'created_timestamp', 'createdAt', 'timestamp']);
  const timestampUtc = normalizeTimestampUtc(timestampRaw);

  const text = getString(message, ['text', 'content', 'message', 'body']) ?? '';

  const type = getString(message, ['type', 'messageType', 'eventType']);
  const isSystem = senderName.toLowerCase() === 'system' || Boolean(type && type !== 'MESSAGE');

  const hasAttachment = hasAnyAttachment(message);

  const messageId = sha256Hex(`goog_chat|${timestampUtc}|${senderName}|${text}`);
...
export class GoogleChatJsonAdapter extends BaseAdapter {
  readonly platform = Platform.GOOG_CHAT;
  ...
  async *parseMessages(input: AdapterInput): AsyncGenerator<IUnifiedMessage> {
    ...
    for await (const chunk of messagesArrayStream as AsyncIterable<StreamArrayChunk>) {
      ...
      yield mapGoogleChatMessageToUnified(chunk.value);
    }
  }
}
