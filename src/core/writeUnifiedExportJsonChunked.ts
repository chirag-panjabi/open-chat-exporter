...
export async function writeUnifiedExportJsonChunked(params: {
    ...
    try {
        for await (const message of params.messages) {
            ...
            firstInChunk = false;
            await writeWithBackpressure(stream!, `${prefix}${JSON.stringify(message)}`);
        }

        await closeChunk();
    } catch (err) {
        // Best-effort close if a stream is open.
        try {
            await closeChunk();
        } catch {
            // ignore
