...
async function main(): Promise<void> {
  ...
  assert(chatInfo.platform === 'SNAPCHAT', 'chat_info.platform should be SNAPCHAT');
  assert(chatInfo.chat_name === 'Snapchat fixture', 'chat_info.chat_name should match override');

  const messages = await readMessages(outputPath);
  assert(messages.length === 4, `Expected 4 messages, got ${messages.length}`);

  const m1 = messages.find((m) => typeof m.content === 'string' && m.content.includes('Hello from Snapchat fixture.'));
