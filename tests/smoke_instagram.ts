...
async function main(): Promise<void> {
  const fixturePath = 'tests/fixtures/instagram/instagram.sample.json';
  const outputPath = join(tmpdir(), `unified-chat-exporter.instagram.${Date.now()}.json`);
  ...
  assert(chatInfo.platform === 'INSTAGRAM', 'chat_info.platform should be INSTAGRAM');
  assert(chatInfo.chat_name === 'Instagram fixture', 'chat_info.chat_name should match override');

  const messages = await readMessages(outputPath);
  assert(messages.length === 4, `Expected 4 messages, got ${messages.length}`);

  const m1 = messages.find((m) => typeof m.content === 'string' && m.content.includes('Hello from Instagram fixture.'));
  ...
  assert(reactions[0].emoji === '❤️', 'Reaction emoji mismatch');
