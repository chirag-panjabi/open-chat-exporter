...
async function main(): Promise<void> {
  const fixturePath = 'tests/fixtures/linkedin/messages.sample.csv';
  const outputPath = join(tmpdir(), `unified-chat-exporter.linkedin.${Date.now()}.json`);
  ...
  assert(chatInfo.platform === 'LINKEDIN', 'chat_info.platform should be LINKEDIN');
  assert(chatInfo.chat_name === 'LinkedIn fixture', 'chat_info.chat_name should match override');

  const messages = await readMessages(outputPath);
  assert(messages.length === 3, `Expected 3 messages, got ${messages.length}`);

  const m1 = messages.find((m) => typeof m.content === 'string' && m.content.includes('Hello, Bob'));
  assert(m1, 'Expected message 1 to exist');
  assert(typeof m1.content === 'string', 'Message 1 content should be a string');
  assert(m1.content.includes('multiline message'), 'Expected multiline content to be preserved');
